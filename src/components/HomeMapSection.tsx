...
import { MapPin, Home, Loader2, Navigation, Search, Building2, Eye } from "lucide-react";
    ...
    const fetchApiKey = async () => {
      try {
        ...
        if (data?.apiKey) {
          ...
          setError('لم يتم تكوين مفتاح API للخرائط');
        }
      } catch (error) {
        console.error('Error fetching API key:', error);
        setError('حدث خطأ في تحميل الخرائط');
      }
    };
    fetchApiKey();
  }, []);
        ...
        <div className="container">
          ...
          <Card className="overflow-hidden">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-2">يرجى التأكد من تكوين مفتاح Google Maps API</p>
            </CardContent>
          </Card>
        </div>
      </section>
          ...
          <div className="relative h-[500px]">
            {loading || !apiKey ? (
              ...
              <>
                ...
                {selectedProperty && (
                  <Card className="absolute bottom-4 right-4 left-4 md:left-auto md:w-80 z-10 shadow-lg">
